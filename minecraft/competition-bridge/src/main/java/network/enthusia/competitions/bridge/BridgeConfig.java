...
public record BridgeConfig(
    ...
    public static BridgeConfig load(CompetitionBridgePlugin plugin) {
        ...
        String bearer = firstNonBlank(
                ...
                config.getString("security.bearer-token", "")
        );
        String hmac = firstNonBlank(
                System.getenv("ENTHUSIA_COMPETITION_BRIDGE_HMAC_SECRET"),
                config.getString("security.hmac-secret", "")
        );
        if (bearer.length() < 32 || hmac.length() < 32) {
            throw new IllegalArgumentException("Bridge bearer token and HMAC secret must each be at least 32 characters");
        }
        ...
        String lumaGuildPermission = requireIdentifier(config.getString("integrations.lumaguilds-submit-permission", "ENTER_COMPETITIONS"), "integrations.lumaguilds-submit-permission").toUpperCase(Locale.ROOT);
        ...
        return new BridgeConfig(
                new Server(bindHost, port, workerThreads, maxRequestBytes, mainThreadTimeoutMs),
                new Security(bearer, hmac, clockSkew, nonceRetention),
                new Integrations(playtimePlugin, guildsPlugin, websiteGuildPermission, lumaGuildPermission),
                ...
                new Rewards(
                        ...
                        config.getString("rewards.lore-items.fallback-command", "") == null
                                ? ""
                                : config.getString("rewards.lore-items.fallback-command", "").trim()
                ),
                new Logging(
    ...
    private static String requireText(String value, String path, int max) {
        ...
        if (normalized.isEmpty() || normalized.length() > max) {
            throw new IllegalArgumentException(path + " is missing or too long");
        }
        return normalized;
    }
    ...
    public record Server(String bindHost, int port, int workerThreads, int maxRequestBytes, int mainThreadTimeoutMs) {}
    public record Security(String bearerToken, String hmacSecret, int maxClockSkewSeconds, int nonceRetentionSeconds) {}
    public record Integrations(String playtimePlugin, String guildsPlugin, String websiteGuildSubmitPermission, String lumaGuildsSubmitPermission) {}
    ...
    public record Rewards(CommandPolicy commandPolicy, PermissionCommands permissions, RankCommands ranks, String loreItemFallbackCommand) {}
