...
test("staff appeal workspace is private and uses its maintained script", async () => {
  ...
  assert.match(html, /src="\.\.\/assets\/reviewer-appeals\.js\?v=2"/);
  assert.doesNotMatch(html, /<script type="module">/);
});

test("account page manages links outside the competitions workspace", async () => {
  ...
  assert.match(menu, /Account and links/);
