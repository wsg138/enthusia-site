...
try {
  ...
  results.catalogSuggestions = await evaluate("(()=>{const T=window.__MARKET_TEST__,values=T.adapter.suggest('d');return values.length>=10&&values.includes('Diamond')&&values.every(value=>value.toLowerCase().startsWith('d'))})()");
  ...
  if (Object.entries(results).some(([key, value]) => !["counts", "browserErrors", "failedResources", "failedImages", "failedBuildingIds", "mobileViewports"].includes(key) && value !== true) || Object.values(results.mobileViewports || {}).some(value => value !== true) || browserErrors.length || failedResources.length || results.failedImages.length || results.failedBuildingIds.length) process.exitCode = 1;
} finally {
