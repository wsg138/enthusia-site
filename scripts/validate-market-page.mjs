...
try {
  ...
  results.ownerVisualLocations = await evaluate("(()=>{const T=window.__MARKET_TEST__,stall=T.adapter.snapshot.stalls.find(value=>value.owner.type==='PLAYER');T.openBuilding(T.layout.buildings.find(value=>value.stallIds.includes(stall.id)));const card=Boolean(document.querySelector('.stall-card .owner-image'));T.openStall(stall);const drawer=Boolean(document.querySelector('.stall-hero .owner-image'));T.executeSearch('diamond');const search=Boolean(document.querySelector('.result-owner .owner-image'));return card&&drawer&&search})()");
  ...
  results.imageFailureFallback = await evaluate("(async()=>{const T=window.__MARKET_TEST__,Original=window.Image,host=document.createElement('div');class FailedImage{constructor(){this.dataset={}}set src(value){queueMicrotask(()=>this.onerror?.())}}window.Image=FailedImage;host.innerHTML=T.ownerVisual({type:'PLAYER',uuid:'00000000-0000-4000-8000-000000000099',name:'SyntheticFailure',avatar:{kind:'MINECRAFT_HEAD',source:'JAVA',includesOuterLayer:true}});document.body.append(host);T.hydrateOwnerVisuals(host);await Promise.resolve();window.Image=Original;const ok=Boolean(host.querySelector('.player-head.fallback[data-head-failed=true] img'))&&!host.querySelector('.resolved-head');host.remove();return ok})()");
