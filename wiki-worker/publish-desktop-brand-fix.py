...
OUT = Path(os.environ.get('WIKI_DESKTOP_BRAND_OUT', 'wiki-desktop-brand-output'))
BACKUP_MANIFEST = OUT / 'full-backup' / 'manifest.json'
CSS_SOURCE = Path('wiki-worker/mobile-shell.css')
CSS_START = '/* BEGIN ENTHUSIA MOBILE UX */'
CSS_END = '/* END ENTHUSIA MOBILE UX */'
UA = 'EnthusiaWikiDesktopBrandFix/1.0 (owner-authorized wiki publisher)'
...
def managed(existing, block, start, end):
    if start not in existing or end not in existing:
        raise RuntimeError(f'Managed block markers missing: {start} / {end}')
    payload = f'{start}\n{block.strip()}\n{end}'
...
def edit_exact(csrf, title, target, bmap, summary, contentmodel):
    ...
    return {
        'title': title, 'result': 'published', 'oldrevid': edit.get('oldrevid'),
...
def main():
    ...
    css_source = CSS_SOURCE.read_text(encoding='utf-8')
    if '.enthusia-site-brand {\n  display: none;' not in css_source:
        raise RuntimeError('Desktop hide rule missing from durable CSS source')
    if '@media (max-width: 800px)' not in css_source or 'display: inline-flex;' not in css_source:
        raise RuntimeError('Mobile brand restore rule missing from durable CSS source')
    css_target = managed(css_existing, css_source, CSS_START, CSS_END)
    css = edit_exact(
        csrf, 'MediaWiki:Common.css', css_target, bmap,
        'Hide duplicate Enthusia mobile brand on desktop', 'css'
    )
    evidence = {
    ...
    (OUT / 'desktop-brand-fix-evidence.json').write_text(json.dumps(evidence, indent=2) + '\n', encoding='utf-8')
...
if __name__ == '__main__':
    try:
        ...
        print(f'DESKTOP BRAND PUBLISH ERROR: {exc}', file=sys.stderr)
        sys.exit(1)
