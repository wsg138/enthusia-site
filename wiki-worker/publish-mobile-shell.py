...
OUT = Path(os.environ.get('WIKI_MOBILE_OUT', 'wiki-mobile-output'))
...
UA = 'EnthusiaWikiMobileUX/1.0 (owner-authorized wiki publisher)'
...
def main():
    ...
    if 'BEGIN ENTHUSIA GLOBAL THEME' not in css_existing or 'data-enthusia-color-scheme' not in css_existing:
        raise RuntimeError('Approved global theme marker missing from MediaWiki:Common.css')
    ...
    css = edit_exact(
        ...
        'Improve Enthusia mobile layout, navigation and branding', 'css'
    )

    js_before = page('MediaWiki:Common.js')
    guard(js_before, bmap)
    js_existing = js_before.get('content', '')
    if JS_START not in js_existing or JS_END not in js_existing:
        raise RuntimeError('Approved global theme marker missing from MediaWiki:Common.js')
    js_target = managed(js_existing, JS_SOURCE.read_text(encoding='utf-8'), JS_START, JS_END)
    js = edit_exact(
        csrf, 'MediaWiki:Common.js', js_target, bmap,
        'Add Enthusia mobile menu, theme control and header brand', 'javascript'
    )
    ...
    (OUT / 'mobile-shell-evidence.json').write_text(json.dumps(evidence, indent=2) + '\n', encoding='utf-8')
...
if __name__ == '__main__':
    try:
        ...
        print(f'MOBILE SHELL PUBLISH ERROR: {exc}', file=sys.stderr)
        sys.exit(1)
