...
OUT = Path(os.environ.get('WIKI_MINERVA_MENU_OUT', 'wiki-minerva-mobile-sidebar-output'))
...
UA = 'EnthusiaWikiMinervaMobileSidebar/1.0 (owner-authorized wiki publisher)'
...
def main():
    ...
    if sidebar.get('content', '').rstrip() != expected_sidebar:
        raise RuntimeError('Live MediaWiki:Sidebar no longer matches approved public-sidebar.wiki; refusing mobile navigation publication')

    corrections = JS_CORRECTIONS.read_text(encoding='utf-8')
    native_css = CSS_NATIVE.read_text(encoding='utf-8')
    required_js = ['#mw-mf-page-left', '#main-menu-input', 'SIDEBAR_GROUPS', 'makeMinervaNavigation', 'openMinervaMenu']
    for marker in required_js:
        if marker not in corrections:
            raise RuntimeError(f'Minerva correction source missing required marker: {marker}')
    if 'body.skin-minerva:has(#main-menu-input:checked)' not in native_css:
        raise RuntimeError('Native mobile CSS is not using the Minerva checkbox as authoritative open state')
    ...
    js_result = edit(csrf, common_js, target_js, 'Target the real Minerva mobile sidebar and menu control', 'javascript')

    css_now = page('MediaWiki:Common.css')
    guard(css_now, bmap)
    css_result = edit(csrf, css_now, target_css, 'Style the real Minerva mobile sidebar and menu state', 'css')
    ...
    (OUT / 'minerva-mobile-sidebar-evidence.json').write_text(json.dumps(evidence, indent=2) + '\n', encoding='utf-8')
...
if __name__ == '__main__':
    try:
        ...
        print(f'MINERVA MOBILE SIDEBAR PUBLISH ERROR: {exc}', file=sys.stderr)
        sys.exit(1)
