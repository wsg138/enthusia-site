...
OUT=Path(os.environ.get('WIKI_ICON_OUT','wiki-card-icon-style-output'))
OUT.mkdir(parents=True,exist_ok=True)
FILES=['Commands.svg','Market.svg','Warzone.svg','HistoryLore.svg']
COMMENT='Match Explore Enthusia icons to original style'
jar=http.cookiejar.CookieJar()
opener=urllib.request.build_opener(urllib.request.HTTPCookieProcessor(jar))

def api(params,method='POST'):
    full={'format':'json','formatversion':'2','maxlag':'5',**params}
    headers={'User-Agent':'EnthusiaWikiIconStyle/1.0','Accept':'application/json'}
    if method=='GET':
...
def publish(name,csrf):
    data=(ROOT/name).read_bytes()
    local_sha1=hashlib.sha1(data).hexdigest()
    before=imageinfo(name)
    if before.get('sha1') == local_sha1:
        return {'name':name,'sha1':local_sha1,'before':before,'after':before,'result':'already_current'}
    ...
    req=urllib.request.Request(API,data=body,headers={'User-Agent':'EnthusiaWikiIconStyle/1.0','Content-Type':f'multipart/form-data; boundary={boundary}','Accept':'application/json'})
    ...
    return {'name':name,'sha1':local_sha1,'before':before,'after':after,'result':'uploaded'}

def main():
    who,csrf=login()
    uploads=[publish(name,csrf) for name in FILES]
