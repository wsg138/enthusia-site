...
function sanitize(html) {
  ...
  return `<div class="enthusia-wiki">\n${out.trim()}\n</div>\n`;
}

function mainPage() {
  const cards = [
    ['Staff.png','Staff','Staff'],
    ['Guilds.png','Guilds','Noteable Guilds'],
    ['Players.png','Players','Noteable Players'],
    ['Betas.png','Betas','Betas'],
    ['Builds.png','Builds','Builds'],
    ['Mapart.png','Mapart','Maparts'],
    ['Events.png','Events','Events'],
    ['Mechanics2.png','Mechanics','Mechanics'],
  ];
  ...
  return `<div class="enthusia-wiki enthusia-home">
...
const css = `/* BEGIN ENTHUSIA WIKI V2 */
...
for (const [id, page] of Object.entries(pages)) {
  ...
  const content = sanitize(page.body);
  const filename = `${id}.wiki`;
  fs.writeFileSync(path.join(OUT, filename), content);
  manifest.push({ id, title, filename, summary: page.summary || '', section: page.section || '' });
}
...
fs.writeFileSync(path.join(OUT, 'Common.css'), css + '\n');
manifest.push({ id: 'common-css', title: 'MediaWiki:Common.css', filename: 'Common.css', managedSection: true, contentModel: 'css' });
