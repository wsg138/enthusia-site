...
OUT = Path(os.environ.get('WIKI_MOBILE_FIX_OUT', 'wiki-mobile-fix-output'))
BACKUP_MANIFEST = OUT / 'full-backup' / 'manifest.json'
CSS_SOURCE = Path('wiki-worker/mobile-corrections.css')
JS_SOURCE = Path('wiki-worker/mobile-corrections.js')
CSS_START = '/* BEGIN ENTHUSIA MOBILE CORRECTIONS */'
CSS_END = '/* END ENTHUSIA MOBILE CORRECTIONS */'
JS_START = '/* BEGIN ENTHUSIA MOBILE CORRECTIONS */'
JS_END = '/* END ENTHUSIA MOBILE CORRECTIONS */'
UA = 'EnthusiaWikiMobileCorrections/1.0 (owner-authorized wiki publisher)'
...
def main():
    ...
    if 'BEGIN ENTHUSIA GLOBAL THEME' not in css_before.get('content', '') or 'BEGIN ENTHUSIA MOBILE UX' not in css_before.get('content', ''):
        raise RuntimeError('Expected approved global/mobile CSS markers are missing')
    css_target = managed(css_before.get('content', ''), CSS_SOURCE.read_text(encoding='utf-8'), CSS_START, CSS_END)
    css = edit_exact(csrf, 'MediaWiki:Common.css', css_target, bmap,
                     'Polish Enthusia mobile branding, card density, and dark mode', 'css')

    js_before = page('MediaWiki:Common.js')
    guard(js_before, bmap)
    if 'BEGIN ENTHUSIA GLOBAL THEME' not in js_before.get('content', ''):
        raise RuntimeError('Expected approved global JS marker is missing')
    js_target = managed(js_before.get('content', ''), JS_SOURCE.read_text(encoding='utf-8'), JS_START, JS_END)
    js = edit_exact(csrf, 'MediaWiki:Common.js', js_target, bmap,
                    'Polish Enthusia mobile logo and reinforce theme/navigation fixes', 'javascript')
    ...
    (OUT / 'mobile-corrections-evidence.json').write_text(json.dumps(evidence, indent=2) + '\n', encoding='utf-8')
...
if __name__ == '__main__':
    try:
        ...
        print(f'MOBILE CORRECTIONS PUBLISH ERROR: {exc}', file=sys.stderr)
        sys.exit(1)
