...
['<code>/rep</code>','Open your reputation profile.'],['<code>/rep &lt;player&gt;</code>','Open another player\'s profile and leave/edit a review.'],['<code>/rep give &lt;player&gt; &lt;category&gt; &lt;reason&gt;</code>','Direct command alternative to the profile menu.'],['<code>/rep top</code> / <code>bottom</code>','Open reputation leaderboards.'],['<code>/rep reviews [player]</code>','View recent reviews.']])}
<h2>Reputation effects</h2><p>Reputation can change gameplay at certain scores. Positive and negative effects are shown below so players can see exactly what a score does.</p>
...
'Death Duel participants are exempt from reputation effects that would distort the duel rules.'
])}
...
${table([
['<code>/rep stalk &lt;player&gt; [days]</code>','Buy a subscription for an eligible player.'],['<code>/rep stalk list</code>','View your active subscriptions and remaining time.'],['<code>/rep stalk cancel &lt;player&gt;</code>','Cancel one.']])}
`};
...
<h2>Tags</h2><p><code>/tags</code> shows the tags you actually own. Click a tag to equip it; only one tag can be selected at a time. The menu also lets you clear your current tag.</p>
<h2>Achievements and rewards</h2><p>The reward catalog contains roughly 100 configured achievements across <strong>Playtime, Mining, Combat, Deaths, Economy and Misc</strong>.</p>
<p>Rewards can give tags, Raw Gold, Minecraft items, cosmetics/permission unlocks and other configured rewards.</p>
${details('Example rewards','a few examples from the current catalog',table([
...
<h2>How progress is counted</h2><p>Different rewards can use different kinds of progress. Active playtime, total playtime, consecutive active time and active time underground can be separate requirements.</p>
...
<p>Missing a calendar day resets the next claim to Day 1. The server also limits normal daily claiming to one account per shared IP each day, with staff-managed household/sibling exceptions where appropriate.</p>
...
<p>Locked cosmetics remain visible but cannot be selected until you earn the required unlock. Current cosmetic types include projectile trails, kill/death effects, player trails, custom PvP kill messages and custom join/quit messages.</p>
`};

if(P.mechanics) P.mechanics.body=P.mechanics.body
.replace('<p>These are the mechanics players are most likely to need, build around, or look up. Automatic systems are included when they meaningfully change normal survival.</p>','<p>Enthusia adds systems for base privacy, travel, trading, PvP, progression and community play. Major mechanics have their own pages below.</p>')
.replace('link(\'java-bedrock\'','link(\'java-bedrock\'');
