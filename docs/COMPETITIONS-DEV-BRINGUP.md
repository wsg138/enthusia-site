...
The Discord webhook must be an HTTPS `discord.com`/`discordapp.com` webhook URL. The application validates the host/path and never returns the URL through the admin status endpoint.
...
Copy `cloudflare/competition-jobs/wrangler.example.jsonc` to an ignored dev config and set the same development D1 ID. Put the bridge bearer/HMAC secrets and Discord staff webhook variables into that Worker as secrets/variables too. The scheduler has no public fetch handler and `workers_dev` is disabled.
...
6. Verify image metadata cleanup, OpenAI moderation failure-closed behavior, reordering, cover selection, and max-eight-image enforcement.
7. Verify contributor invites/reminders and acceptance; once voting begins, roster changes are locked except already-pending invitation responses.
...
9. Verify Needs Changes, Approve, Reject, Disqualify, image removal, whole-entry removal/restore, staff edits, manual player entry creation, and Gallery promotion/removal.
10. Verify one Discord identity gets one ballot across all linked Minecraft accounts; linked alts cannot self-vote, bypass judge rules, or receive judge-excluded rewards.
11. Verify active-playtime voting eligibility is sourced from PlayTimePlugin.
12. Verify judge coordinate access is absent unless explicitly enabled, and assigned judges cannot submit or public-vote.
13. Verify provisional standings/results are staff-reviewed before explicit result publication.
