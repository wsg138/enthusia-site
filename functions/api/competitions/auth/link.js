...
async function startMinecraftLink(context, session) {
  try {
    const limited = await linkStartRateLimit(context, session);
    ...
    try {
      ...
      ).bind(link.codeHash, session.discord.id).run().catch(() => {});
      throw error;
    }
    return json({
...
async function pollMinecraftLink(context, session, input) {
...
async function unlinkMinecraft(context, session, input) {
...
function linkAction(context, session, input) {
  switch (input?.action) {
    case "START":
      return startMinecraftLink(context, session);
    case "POLL":
      return pollMinecraftLink(context, session, input);
    case "UNLINK":
      return unlinkMinecraft(context, session, input);
    default:
...
export async function onRequestPost(context) {
  ...
  return linkAction(context, authorized.session, input);
}
